...
import { ScrollText, Trash2, HistoryIcon } from 'lucide-react';
import ChatMessage from '@/components/chat/ChatMessage';
import type { ChatSession } from '@/types/chat';
import { getAllChatSessions, clearChatHistory as clearHistoryUtil } from '@/lib/localStorage';
...
import LoadingSpinner from '@/components/chat/LoadingSpinner'; // Assuming you have this
          ...
          {sessions.length === 0 ? (
            ...
            <Accordion type="single" collapsible className="w-full space-y-2">
              {sessions.map((session, index) => (
                <AccordionItem value={`session-${session.id}-${index}`} key={session.id} className="bg-muted/30 rounded-lg border shadow-sm">
                  <AccordionTrigger className="px-4 py-3 hover:bg-muted/50 rounded-t-lg text-sm font-medium text-primary">
                    ...
                    <div className="space-y-2 max-h-[60vh] overflow-y-auto pr-2"> {/* Added scroll for long sessions */}
                      {session.messages.map((msg) => (
                        <ChatMessage
                          key={msg.id}
                          message={msg}
                          isHistoryView={true} // Indicate this is for history display
                          // No interactive props needed for history view
                        />
                      ))}
                    </div>
                  </AccordionContent>
